...
out_file = 'P://osm/output/modified_all.osm'
orig_file = 'P://osm/output/orig_all.osm'
...
##      child.append(Element('tag', {'k': 'recent', 'v': 'true'}))
    ...
    print "searching for nodes in required ways for relations"
    for child in root.findall('way'):
        
        if(child.attrib['id'] in required_ways):
            for sub_element in child.iter('nd'):
                required_nodes.append(sub_element.attrib['ref'])
        else:
            root.remove(child)
